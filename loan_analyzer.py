...
print(f"The total of all loan is: {total_of_loans}")
...
def present_value(future_value, discount_rate, remaining_months):
...
# YOUR CODE HERE!
def inexpensive_loan_list(loans):
    inexpensive_loans_list = []
    for loan_price in loans:
        if loan_price <= 500:
            inexpensive_loans_list.append(loans)
    print (inexpensive_loans_list)


def inexpensive_loan_list():
    print(loans)
inexpensive_loan_list()   
...
output_path = Path("inexpensive_loans_list.csv")
...
with open(csvpath) as csvfile:
    inexpensive_loan_list = csv.writer(csvfile)
    for loan in inexpensive_loans_list:
        print(loan)
